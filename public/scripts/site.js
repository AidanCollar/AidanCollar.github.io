...
document.querySelector('.container').addEventListener('click',function(e){
    ...
    star.style.top=`-5px`;
    const randomEndx = (Math.random()*1000-500)+'px';
    star.style.setProperty('--random-end-x',randomEndx)
...
const fetchimage = () => {
    ...
    const src='https://r.sine.com/index'
    const newsrc= src.split('?')[0] + '?cacheBust='+ new Date().getTime();
    child.src=newsrc
    child.classList.add('resizedimage')

    imagebox.appendChild(child)
...
document.querySelector('.randomimage').addEventListener('click',fetchimage)
